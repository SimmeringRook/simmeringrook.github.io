...
\begin{align}\label{incorrectShellSpeed}
\frac{dr}{dt} &= - \left(1-\frac{2M}{r}\right)\sqrt{\frac{2M}{r}}\nonumber \\
...
But this relation doesn't behave nicely. The curvature factor term sets the point of inflection for $dr/dt_{bk}$ at $6M$: until this $r$-coordinate, the Bookkeeper measures the stone to fall inwards at an increasing velocity and any velocity beyond this $r$-coordinate decays towards $0$. Equation $(\ref{incorrectShellSpeed})$ will behave nicely for shell observers at or above $6M$, but will lead to unphysical results below this threshold. For $r<6M$, the critical point stops translating to the left (towards the origin), but the function still needs to reach the value as described by $\sqrt{2M/r_{obs}}$ and this corresponds to the critical point translating *downward*, giving speed measurements greater than or equal to $-1$ for $r\neq 2M$. This is a two-fold problem: not only does a massive object obtain the speed of light, it can apparently exceed it!
