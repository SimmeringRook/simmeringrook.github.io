...
Galilean relativity is how we describe relative velocities in everyday life and requires some extremes to find the breaking points. If we consider a GPS satellite orbiting the Earth, the Galilean description works perfectly fine: The satellite at 5m away is orbiting us with a tangential velocity of 5 m/s. This description works perfectly for all of this, but if we start asking questions like: “What time does the atomic clock read on the satellite compared to my own?”, this is where we need to begin to apply the corrections offered in special relativity (SR). 

Through length contraction and time dilation, we can now find a scaling factor such that the measurements made from Earth and those by the satellite agree. However, this correction obtained by changing from measuring spatial and temporal separation as two separate quantities (Galilean) to the one quantity of spacetime (SR) is not the whole picture. SR offers an exact description if there were no curvature to spacetime, something that massive bodies exert in the form of gravity.

To truly fix the disagreement in measurements from Earth and the GPS satellites, such that they can work as intended, we must also consider how the curvature of spacetime effects these two reference frames. The short answer is that since spacetime is contracted more intensely close to the source of a strong gravitational field, we can introduce another set of scaling factors—much like in SR—to resolve this other compounding set of length contraction and time dilation.

This aforementioned model is called Schwarzschild Geometry and is the simplest solution to Einstein’s field equations involving a single massive object that is non-rotating. While we now have a theory that provides extremely accurate predictions, we subtly sacrificed a lot along the way. In Galilean relativity, we could easily switch from frame to frame and agree about what all observers measured: distance, time, relative velocities, and order of events. As the relative speed of objects increased, we had to switch to special relativity, but not everything carries over from Galileo, most notably: observers in SR are not guaranteed to agree on the order in which the see events occurring (see the “barn and pole” paradox). Finally, in using general relativity (GR) to correctly unify observers’ measurements in the presence of a gravitational field, we lose the notion of relative velocity.
