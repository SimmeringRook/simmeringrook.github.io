...
### Space into Spacetime {#spaceIntoSpacetime}
...
The end goal of Cartographer is to generate visualizations corresponding to light and low-mass particles moving through Schwarzschild spacetime. The following subsections provide additional context for each step by detailing the process of implementing the more accessible expressions from $\ref{motivation}$. These subsections focus primarily on the computational implementation whereas the following sections discuss and analyze the physics of the resulting visualizations.
...
> Shorthand should be introduced relatively early into the paper.

We begin by isolating the descriptions of proper time and proper distance as measured by a shell observer using the line element (Equation $\ref{SchwarzschildWithC}$). Recall from $\ref{spaceIntoSpacetime}$ that proper time and distance are invariant with respect to changing reference frames. We introduce the shorthand of $f(r)= \sqrt{1 - \frac{2M}{r}}$ and $d^2\Omega=\sin^2{\theta}d^2\varphi + d^2\theta$ to simplify the line element.

\begin{equation}
ds^2= - f(r)^2 (dt^2) + \frac{dr^2}{f(r)^2} + r^2 d^2\Omega
\end{equation}

Restricting the Schwarzschild coordinates that can vary to $r$ or $t$ allows us to set $d\Omega=0$ Examining change in only $r$ or $t$, we arrive at the description of proper distance,  
...
for a stationary shell observer.
...
Since there is a coordinate singularity at $r=2M$ and shell observers only exist outside the event horizon (see $\ref{massIsCurvature}$), we need to adjust the interval of possible $r$-coordinates accordingly. If represented on a linear-linear plot, the domain cannot span $(2M,\infty)$ as the shape of the function will be supressed. Similarly, the domain must also span enough $r$-coordinates to demonstrate the function's properties.
...
[^-21]: The equivalent analogy here is the caution often given to students about what information they can obtain from figures included with problem statements. **TODO**: Still need a better way to describe this. The gist is: remember how we're cautioned about inferring too much from how the pictures look? That's the danger of comments in code. Just because they are there does not mean they are accurate $\leftrightarrow$ Just like two angles look similar does not mean they are the same, unless given the same variable, or if the algebra works out (which is equivalent to checking to make sure the code does what the comment says!).
